    ...
    while url != []:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                price = await (await page.query_selector_all('div[class="sc-129a3fbb-0 sc-129a3fbb-6 fZObHF gTiXml"]'))[i].text_content()
                ...
                date = await (await page.query_selector_all('div[class="sc-129a3fbb-0 sc-129a3fbb-10 fZObHF cFcOfg"]'))[i].text_content()
                bounding_box = await (await page.query_selector_all('div[class="sc-129a3fbb-0 sc-129a3fbb-10 fZObHF cFcOfg"]'))[i].bounding_box()
                x = bounding_box['x']
                y = bounding_box['y']
                ...
                await (await page.query_selector_all('div[class="sc-45c0335-3 ecqRRC"]'))[i].click() #score
                ...
                if mn_price > int("".join(price.split()[:-1])):
                    mn_price = int("".join(price.split()[:-1]))
                    mn_date = date.split()[0]
                    mn_brand = brand.split()[1][:4]

                # print(i, mn_price, mn_date, mn_brand)
                await (await page.query_selector('body')).click()
            ...
            if mn_date and mn_brand:
                with open('parser/new3.txt', 'a', encoding="UTF-8") as file:
                    file.write(f'{url[0]} | price: {mn_price} | date: {mn_date} | brand: {mn_brand}')
                    file.write('\n')
