...
async def create_filter(filter_in: FilterCreate, session: AsyncSession = Depends(db_helper.session_depends), access_token: str | None = Header(default=None, convert_underscores=False)):
...
async def get_filter(session: AsyncSession = Depends(db_helper.session_depends), access_token: str | None = Header(default=None, convert_underscores=False)):
    payload = await check_payload(access_token=access_token)

    return await crud.get_filter(session=session, user_id=payload.get("sub"))

@router.get("/get_filter/{filter_id}")
async def get_filter_by_id(filter_id: int, session: AsyncSession = Depends(db_helper.session_depends), access_token: str | None = Header(default=None, convert_underscores=False)):
    payload = await check_payload(access_token=access_token)
...
async def edit_filter(filter_id: int, upd_filter: FilterUpdate, session: AsyncSession = Depends(db_helper.session_depends), access_token: str | None = Header(default=None, convert_underscores=False)):
    payload = await check_payload(access_token=access_token)
...
async def delete_router(filter_id: int, session: AsyncSession = Depends(db_helper.session_depends), access_token: str | None = Header(default=None, convert_underscores=False)):
    payload = await check_payload(access_token=access_token)
